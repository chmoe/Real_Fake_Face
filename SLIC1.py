...
class SLICProcessor(object):
    ...
    def iterate_10times(self, j, name):
        self.init_clusters()  # 初始化超像素
        self.move_clusters()
        for _ in trange(10):
            self.assignment()
            self.update_cluster()
        name = path_exist('./SLIC_result/' + name) + '/{}.png'.format(j)
...
def run(list_name, name):
    ...
    for i in range(start_count, file_count):
        print(name + "Image: Processing({}/{})".format(i + 1, file_count))
        p = SLICProcessor(list_name[i], 40, 10)
        p.iterate_10times(i, name)
